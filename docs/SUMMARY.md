# Table of Contents - Pureyes 文档索引

* [帮助中心首页](README.md)

## 1. 终端用户指南 (User Guide)
* [01-系统简介与核心特性](user-guide/01-overview.md)
* [02-客户端快速入门](user-guide/02-quick-start.md)
* [03-账号认证与安全设置](user-guide/03-authentication.md)
* [04-小组协作与团队通讯录](user-guide/04-group-collaboration.md)
* [05-实时视频监控与画面抓拍](user-guide/05-live-monitoring.md)
* [06-工作区管理与视频切片](user-guide/06-workspace-management.md)
* [07-智能多模态视觉问答 (MVA)](user-guide/07-ai-multimodal-qa.md)
* [08-管理员控制台与用户管理](user-guide/08-admin-console.md)
* [09-个人中心与系统设置](user-guide/09-profile-and-settings.md)

## 2. 开发者技术文档 (Developer Guide)
* [01-系统全局架构设计与数据流](developer-guide/01-architecture-design.md)
* [02-鸿蒙前端 ArkTS 架构与组件设计](developer-guide/02-frontend-arkts.md)
* [03-后端 RESTful API 接口规范](developer-guide/03-backend-flask-api.md)
* [04-AI 多模态视觉分析引擎原理](developer-guide/04-ai-mva-engine.md)
* [05-数据库设计与实体关系模型](developer-guide/05-database-schema.md)

## 3. 自建服务器部署手册 (Server Deployment)
* [01-服务器硬件配置、CUDA 与环境依赖](server-deployment/01-requirements-and-env.md)
* [02-后端服务部署、大模型配置与运行维护](server-deployment/02-backend-deployment-guide.md)
